...
void WebsocketSession::on_resolve(const boost::system::error_code& ec, boost::asio::ip::tcp::resolver::results_type results) {
    ...
    if (m_ssl) {
        boost::beast::get_lowest_layer(*m_ssl).async_connect(results, std::bind(&WebsocketSession::on_connect, shared_from_this(), std::placeholders::_1));
    } else {
        boost::beast::get_lowest_layer(*m_socket).async_connect(results, std::bind(&WebsocketSession::on_connect, shared_from_this(), std::placeholders::_1));
    }
}
